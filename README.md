...
- **更新用户信息**：修改用户个人资料
- **修改密码**：更新用户密码
- **单点登录**：使用Redis实现，确保同一用户只能有一个活跃会话
...
- **Redis**：缓存频繁访问的数据，提高性能，用于设备离线检测和单点登录
...
- **离线检测**：使用Redis监测设备是否离线
- **离线预警**：当设备长时间未上报数据时触发预警
- **上线提醒**：当设备重新上线时发送提醒
- **通知次数限制**：可配置最大通知次数，避免频繁打扰

### 设备管理

- **设备绑定**：用户绑定设备，建立用户与设备的关联
- **设备解绑**：解除用户与设备的关联
- **获取设备列表**：查询用户绑定的所有设备
- **设备隔离**：使用user_id + device_id确保不同用户的设备不会混淆

### 设备激活系统

- **激活码生成**：为设备生成唯一激活码
- **激活码限流**：防止激活码被滥用
- **设备绑定验证**：确保设备只能被一个用户绑定

### 通知系统

- **通知记录**：记录所有类型的通知，包括预警、离线提醒、上线提醒等
- **通知查询**：获取用户的通知列表
- **通知标记**：标记通知为已读
- **未读通知计数**：获取用户未读通知数量
